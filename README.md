...
# 1. Copy example configs and fill in your values
...
The `terraform.tfvars` file is gitignored and must contain your GCP project ID, project number, Docker image reference, GCS bucket name, and Turso cache URL. See `terraform.tfvars.example` for the template.

### Secret values

API keys are managed through GCP Secret Manager. After `terraform apply`, populate them:

```bash
echo -n "your-key" | gcloud secrets versions add hevy-api-key --data-file=-
echo -n "your-token" | gcloud secrets versions add rp-bearer-token --data-file=-
echo -n "your-key" | gcloud secrets versions add openrouter-api-key --data-file=-
echo -n "your-token" | gcloud secrets versions add turso-auth-token --data-file=-
```
